...
// Global language preference storage (Bulgarian is the default for new recordings).
static LANGUAGE_PREFERENCE: std::sync::LazyLock<StdMutex<String>> =
    std::sync::LazyLock::new(|| StdMutex::new("bg".to_string()));
...
pub fn run() {
    ...
    builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            connections::api_retry_meeting_sync,
            auth::auth_start_sign_in,
            auth::auth_get_session,
            auth::auth_sign_out,
            auth::auth_open_profile,
            auth::auth_open_organization,
            auth::auth_finish_recording_scope,
